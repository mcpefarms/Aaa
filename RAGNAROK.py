...
bot = telebot.TeleBot('7328559193:AAExACwAXzmzK4m_BfekV1LbjyVu_pax5NQ')
...
def handle_bgmi(message):
    ...
    if user_id in allowed_user_ids:
        ...
        if len(command) == 4:  # Updated to accept target, time, and port
            ...
            response = "Usage :- /attack <target> <port> <time>\nhttps://t.me/golemxddos"  # Updated command syntax
    else:
...
def welcome_plan(message):
    user_name = message.from_user.first_name
    response = f'''{user_name}, Brother Channel Mai Uploaded Hai Prices !!:
https://t.me/golemxddos
'''
    bot.reply_to(message, response)
